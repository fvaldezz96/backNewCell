...
const webHooksFunction = async (req, res) => {
    try {
        ...
        if (data.action === 'payment.created') {
            ...
            // console.log('const arr result:', arr);
            ...
            try {
                ...
                //HAY QUE HACER CAMBIOS EN LA LOGICA DEL STOCK 
                // await Promise.all(cell.map(async (e) => await Cell.update({ stock: e.stock }, { where: { id: e.id } })));
                cell.forEach(e => {
                    Cell.update({ stock: e.stock }, { where: { id: e.id } })
                });
                res.status(200).json({ message: "Successful Payment, stock changed and email sent !!😁" });
            } catch (err) {
